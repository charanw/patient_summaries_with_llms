...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Parameter tuning of LLM on mimic summarization."
    )
    ...
    parser.add_argument("--evaluation_model_path", type=str, default=None)
    parser.add_argument(
        "--train_file",
        type=str,
        default=None,
        help="Path to custom training JSON/JSONL file",
    )
    parser.add_argument(
        "--val_file",
        type=str,
        default=None,
        help="Path to custom validation JSON/JSONL file",
    )
    parser.add_argument(
        "--test_file",
        type=str,
    ...
    parser.add_argument("--save_and_logging_steps", type=int, default=10)
    ...
    parser.add_argument("--num_target_modules", type=int, default=4)
...
def main():
    ...
    if args.evaluation:
        wandb.init(
            project=f"mimic-iv-note-di-bhc_{short_model_name}_evaluation",
            config=args,
            tags=[],
            notes="",
        )
        ...
        wandb.init(
            project=f"mimic-iv-note-di-bhc_{short_model_name}_parameter-tuning",
            config=args,
            tags=[],
            notes="",
        )
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        token=hf_token,
        trust_remote_code=True,  # for mistral,
    )
    tokenizer = AutoTokenizer.from_pretrained(
        model_name, token=hf_token, trust_remote_code=True  # for mistral,
    )
    tokenizer.pad_token = tokenizer.eos_token  # for mistral
    ...
    # Prefer custom files if provided
    if args.train_file:
    ...
    if args.num_train_examples:
        data_train = data_train.select(
            range(0, len(data_train))[-args.num_train_examples :]
        )
    ...
    if args.num_test_examples:
        data_test = data_test.select(
            range(0, len(data_test))[-args.num_test_examples :]
        )
    ...
    # print(
    #     f"Truncated texts: {sum(data_train['truncated'])} train, {sum(data_val['truncated'])} val, {sum(data_test['truncated'])} test"
    # )
        ...
        for ex in dataset:
            ...
            with torch.cuda.amp.autocast():
                generation_output = prediction_model.generate(
                    ...
                    pad_token_id=tokenizer.pad_token_id,  # for mistral
                )
    ...
    if args.evaluation:
        ...
        # Prepare response template for collator without llama 2 workarounds
